...
type GetMyApprovalRoomsHandler struct {
	validate        *validator.Validate
	sugaredErrorMsg *utils.SugaredErrorMessageValidator

	requestsService *approvalroom.ApprovalRoomService
	user            *user.User
}

func NewGetMyApprovalRoomsHandler(
	v *validator.Validate,
	sgr *utils.SugaredErrorMessageValidator,
	ars *approvalroom.ApprovalRoomService,
	usr *user.User,
) *GetMyApprovalRoomsHandler {
	return &GetMyApprovalRoomsHandler{
		validate:        v,
		sugaredErrorMsg: sgr,
		requestsService: ars,
		user:            usr,
...
func (g *GetMyApprovalRoomsHandler) HandleFunc(c *echo.Context) error {
	...
	var q = new(dto.GetMyApprovalRoomsDTO)
	...
	rooms, err := g.requestsService.GetApprovalRoomsBySubmitter(
		c.Request().Context(),
		currentUser.ID,
		sortField,
		orderDir,
